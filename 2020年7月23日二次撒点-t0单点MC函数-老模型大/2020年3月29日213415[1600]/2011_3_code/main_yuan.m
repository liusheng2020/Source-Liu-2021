...
%dB=[0:10:50]' ;
dB=[0:5:25]' ;
compare_data=zeros(8,length(dB));
dB2xigma;
...
%% 求CRLB和各算法的定位精度
solve_R_CRLB;
compare_data=[compare_data;R_CRLB']

%% 各算法的时间复杂度
mean_T=[mean(T_Monte_carlo_nonzhixin);mean(T_Monte_carlo_zhixin);mean(T_TR_nonzhixin);mean(T_TR_zhixin);mean(T_2LS);mean(T_MMA);mean(T_2016);mean(T_2013)]
T=[mean_T(1,:)+mean_T(3,:);mean_T(2,:)+mean_T(4,:);mean_T(5:8,:)];
...
%% 绘图
figure(2)
curve=semilogy(dB,compare_data(1,:),'--+g',dB,compare_data(2,:),'--or',dB,compare_data(3,:),'-.*g',dB,compare_data(4,:),'-.sr',dB,compare_data(5,:),':*b',dB,compare_data(6,:),'-.xm',dB,compare_data(7,:),'-.+k',dB,compare_data(8,:),'-.xk',dB,compare_data(9,:),'-.ok');
ylabel('RMSE(m)');
xlabel('1/\sigma^{2}(dB)');
grid on;
%legend('show')
legend('Monte carlo-非质心','Monte carlo-质心','The proposed MC-TR-非质心','The proposed MC-TR-质心','2LS','MMA','SDP NEW 2016','SDP NEW 2013','ROOT OF CRLB')
