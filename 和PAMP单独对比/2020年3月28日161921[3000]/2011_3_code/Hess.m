...
e=sum(d,2)/(anchors_n*cspeed);%2X1
f=mean((ti'-c(1,:)/cspeed),2);
I=eye(2);
p=[0 0;0 0];
for m=1:anchors_n
    p=p+(a(:,m)*a(:,m)'-c(1,m)^2*I)/c(1,m)^3/cspeed;
end
pmean=p/anchors_n;
h1=[0 0;0 0];
h2=[0 0;0 0];
for m=1:anchors_n
    g=d(:,m)/cspeed-e;
    h1=h1+g*g';
    h2=h2+(ti(m,1)-c(1,m)/cspeed-f)*((a(:,m)*a(:,m)'-c(1,m)^2*I)/c(1,m)^3/cspeed-pmean);
end
He=2*(h1+h2);
% 
% 
