...
R_CRLB=zeros(runnum,length(dB));
R_CRLB_TDOA=zeros(runnum,length(dB));
...
for kkk=1:length(dB)
    ...
    for kk=1:runnum
        ...
        %% 运行各算法
        %Scenario 2, small t0: MDMTM+TR, 2LS, MMA, SDP 2013
        ...
        %SDP_NEW_2016; 
        SDP_NEW_2013;
        [LocTDOA(:,kk),T_TDOA(kk,kkk)]=TDOA2005(Kn2,xigma);
        
        
        
        R_CRLB(kk,kkk)=solve_R_CRLB(xigma);
        R_CRLB_TDOA(kk,kkk)=solve_R_CRLB_TDOA(xigma);
...
Area2=mean(Area1)
%% 求CRLB和各算法的定位精度
R_CRLB_vector=mean(R_CRLB,1);
R_CRLB_TDOA_vector=mean(R_CRLB_TDOA,1);
compare_data=[compare_data;R_CRLB_vector;R_CRLB_TDOA_vector]
...
curve=semilogy(dB,compare_data(2,:),'--dr',dB,compare_data(4,:),'-.>r',dB,compare_data(5,:),'--sb',dB,compare_data(6,:),'-.+m',dB,compare_data(8,:),':<k',dB,compare_data(9,:),':sg',dB,compare_data(10,:),':*b',dB,compare_data(11,:),'--ok',dB,compare_data(12,:),':oc');

ylabel('RMSE(m)');
xlabel('1/\sigma^{2}(dB)');
grid on;
%legend('show')
legend('MC (Barycenter)','Proposed MC-TR','2LS in [15]','MMA in [15]','SDP 2013 in [19]','Classic TDOA in [2]','PAMP in [20]','CRLB-TOA','CRLB-TDOA','Location','southwest')
%axis([0 25 0.07 10]);
ylim([0.07 10]);
