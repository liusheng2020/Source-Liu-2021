...
for kkk=1:length(dB)
    ...
    for kk=1:runnum
        ...
        %% 运行各算法
        % Scenario 3, small t0:  MDMTM+TR, 2LS, MMA
...
curve=semilogy(dB,compare_data(2,:),'--dr',dB,compare_data(4,:),'-.>r',dB,compare_data(5,:),'--sb',dB,compare_data(6,:),'-.+m',dB,compare_data(9,:),':sg',dB,compare_data(10,:),':*b',dB,compare_data(11,:),'--ok',dB,compare_data(12,:),':oc');

ylabel('RMSE(m)');
xlabel('1/\sigma^{2}(dB)');
grid on;
%legend('show')
%legend('MC B','MC B TR','2LS','MMA','ROOT OF CRLB')
legend('MC (Non-Barycenter)','Proposed MC-TR','2LS in [15]','MMA in [15]','Classic TDOA in [2]','PAMP in [20]','CRLB-TOA','CRLB-TDOA','Location','southwest')
