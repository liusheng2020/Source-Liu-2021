...
test_number_1=ceil(8*(1600*1600/(pi*80^2)));
...
for i=1:test_number_1
    ...
    test_loc=[unifrnd(-800, 800, 1, 1);unifrnd(-800, 800, 1, 1)];
...
legend('Anchor node','Source node','MC points','1-st','2-nd','3-rd','4-th','5-th','6-th and above')
title(['The First MDTM-based Monte Carlo Search'])
ylabel('m');
xlabel('m');
hold off
saveas(gcf,['kkk=',num2str(kkk),'_kk=',num2str(kk),'A'],'epsc'); %保存当前窗口的图像
pause(3)
%}
...
%% 在凸包内二次撒点
test_number_2=ceil(5*area/(pi*8^2));%平均每个小方格120个测试点
...
legend('Anchor node','Source node','MC points','1-st','2-nd','3-rd','4-th','5-th','p_0','First search')
title(['The Second MDTM-based Monte Carlo Search'])
ylabel('m');
xlabel('m');
hold off
saveas(gcf,['kkk=',num2str(kkk),'_kk=',num2str(kk),'B'],'epsc'); %保存当前窗口的图像
pause(3)
%}